...
const ChatContainer = ({selectedUser,setSelectedUser}) => {
    // used to scroll using the useref at the end og tha chat area          
  const scrollEnd=useRef()
  useEffect(()=>{
if(scrollEnd.current){
  scrollEnd.current.scrollIntoView({behaivor:"smooth"})
}},[])

  return selectedUser ?(
    <div className='h-full overflow-scroll reletive backdrop-blur-lg'>
   {/*-----------header---------------*/ }
             ...
             <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3
              ...
              {messagesDummyData.map((msg, index)=>(
                ...
                senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`} >
                  {msg.image ? (
                    <img scr={assets.img} alt="" className='max-w-[230px] border
           ...
           <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                  ...
                  text-white placeholder-gray-400 '/>
                   ...
                   <label htmlFor="image">
                    <img src ={assets.gallery_icon} alt="" className="w-5 mr-2 curser-pointer" />
                   </label>
                </div>
                <img src={assets.send_button} alt="" className="w-7 curser-pointer"/>
           </div>
    </div>
  ...
  bg-white/10 max-md:hidden'>  
    <img src={assets.logo_icon} className='max-w-16'  alt=""/>             
          ...
          <p className='text-lg front-medium text-white'>Feel free to chat</p>   
                 </div>
